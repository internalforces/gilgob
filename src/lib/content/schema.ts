...
export const portfolioSchema = z.object({
  ...
  metrics: z.array(portfolioMetricSchema).min(2).max(4),
  story: z.object({
    problem: z.string().min(1),
    approach: z.string().min(1),
    result: z.string().min(1),
  }),
  capabilities: z.array(portfolioCapabilitySchema).min(2).max(4),
  ownership: z.array(z.string().min(1)).min(1),
  architecture: z.array(portfolioArchitectureNodeSchema).min(2).max(4),
  decisions: z.array(portfolioDecisionSchema).min(2).max(4),
  validation: z.object({
    steps: z.array(z.string().min(1)).min(3).max(6),
    proofs: z.array(portfolioProofSchema).min(2).max(4),
